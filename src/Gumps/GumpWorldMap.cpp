    ...
    : CGump(GT_WORLD_MAP, 0, x, y)
{
    ...
    //Map settings
    static const astr_t mapNames[7] = { "Current map", "Britannia", "Trammel", "Illshenar",
                                        "Malas",       "Tokuno",    "TerMur" };

    //Scale settings
    static const astr_t scaleNames[7] = { "4:1", "2:1", "1:1", "1:2", "1:4", "1:6", "1:10" };
...
void CGumpWorldMap::LoadMap(int map)
{
    ...
    if (g_MapTexture[map].Texture == 0)
    {
        ...
        auto path = g_App.FilePath("data/worldmap%08X.xuo", crc32);
        bool fromFile = false;
        ...
        if (!fromFile)
        {
            ...
            fs_path_create(g_App.FilePath("data"));
