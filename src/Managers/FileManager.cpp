...
vector<uint8_t> CUopMappedFile::GetData(const CUopBlockHeader &block)
{
    ...
    if ((compressedSize != 0u) && compressedSize != decompressedSize)
    {
        ...
        if (z_err != Z_OK)
        {
            ERROR(Data, "uncompress error: {}", z_err);
            result.clear();
        }
    }
    else
...
bool CFileManager::FileExists(const os_path &filename)
{
    ...
    INFO(Filesystem, "FileExists: {} = {}", CStringFromPath(filename), r);
    return r;
}
    ...
    UOPAnimationData &animData, vector<uint8_t> &decLayoutData, char *buf)
{
    ...
    if (z_err != Z_OK)
    {
        ERROR(Data, "UOP anim decompression failed {}", z_err);
        ERROR(Data, "  anim file: {}", CStringFromPath(animData.path));
        ERROR(Data, "  anim offset: {}", animData.offset);
        return false;
    }
    return true;
}
...
{
    INFO(Data, "loading UOP file: {}", fileName);
    ...
    {
        WARN(Data, "UOP file '{}' formatID is {}", fileName, formatID);
        return false;
    }
    ...
    if (formatVersion > 5)
    {
        WARN(Data, "UOP file '{}' version is {}", fileName, formatVersion);
    }
    ...
    {
        INFO(Data, "item dump start: {:0>16x}, {}", i->first, i->second.CompressedSize);
...
{
    INFO(Filesystem, "trying to open file stream for {}", CStringFromPath(filePath));
    if (!FileExists(filePath))
    {
        WARN(Filesystem, "{} doesnt exist", CStringFromPath(filePath));
        return false;
    }
    fileStream.open(filePath, std::ios::binary | std::ios::in);
    INFO(Filesystem, "opened file stream for {}", CStringFromPath(filePath));
    return true;
}
