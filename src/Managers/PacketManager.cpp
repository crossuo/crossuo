...
CPacketInfo CPacketManager::m_Packets[0x100] = {
    /*0x00*/ SMSG(XUO_SAVE_PACKET, "Create Character", 0x68),
    /*0x01*/ SMSG(XUO_SAVE_PACKET, "Disconnect", 0x05),
    /*0x02*/ SMSG(XUO_IGNORE_PACKET, "Walk Request", 0x07),
    /*0x03*/ BMSGH(XUO_SAVE_PACKET, "Client Talk", PACKET_VARIABLE_SIZE, ClientTalk),
    /*0x04*/ BMSG(XUO_SAVE_PACKET, "Request God mode (God client)", 0x02),
    /*0x05*/ SMSG(XUO_IGNORE_PACKET, "Attack", 0x05),
    /*0x06*/ SMSG(XUO_IGNORE_PACKET, "Double Click", 0x05),
    /*0x07*/ SMSG(XUO_SAVE_PACKET, "Pick Up Item", 0x07),
    /*0x08*/ SMSG(XUO_SAVE_PACKET, "Drop Item", 0x0e),
    /*0x09*/ SMSG(XUO_IGNORE_PACKET, "Single Click", 0x05),
    /*0x0A*/ BMSG(XUO_SAVE_PACKET, "Edit (God client)", 0x0b),
    /*0x0B*/ RMSGH(XUO_IGNORE_PACKET, "Damage Visualization", 0x07, Damage),
    /*0x0C*/ BMSG(XUO_SAVE_PACKET, "Edit tiledata (God client)", PACKET_VARIABLE_SIZE),
    /*0x0D*/ BMSG(XUO_SAVE_PACKET, "Edit NPC data (God client)", 0x03),
    /*0x0E*/ BMSG(XUO_SAVE_PACKET, "Edit template data (God client)", 0x01),
    /*0x0F*/ UMSG(XUO_SAVE_PACKET, 0x3d),
    /*0x10*/ BMSG(XUO_SAVE_PACKET, "Edit hue data (God client)", 0xd7),
    /*0x11*/ RMSGH(XUO_IGNORE_PACKET, "Character Status", PACKET_VARIABLE_SIZE, CharacterStatus),
    /*0x12*/ SMSG(XUO_IGNORE_PACKET, "Perform Action", PACKET_VARIABLE_SIZE),
    /*0x13*/ SMSG(XUO_IGNORE_PACKET, "Client Equip Item", 0x0a),
    /*0x14*/ BMSG(XUO_SAVE_PACKET, "Change tile Z (God client)", 0x06),
    /*0x15*/ BMSG(XUO_SAVE_PACKET, "Follow", 0x09),
    /*0x16*/ RMSGH(XUO_SAVE_PACKET, "Health status bar update (0x16)", 0x01, NewHealthbarUpdate),
    /*0x17*/
    RMSGH(
        XUO_IGNORE_PACKET,
        ...
        NewHealthbarUpdate),
    /*0x18*/ BMSG(XUO_SAVE_PACKET, "Add script (God client)", PACKET_VARIABLE_SIZE),
    /*0x19*/ BMSG(XUO_SAVE_PACKET, "Edit NPC speech (God client)", PACKET_VARIABLE_SIZE),
    /*0x1A*/ RMSGH(XUO_SAVE_PACKET, "Update Item", PACKET_VARIABLE_SIZE, UpdateItem),
    /*0x1B*/ RMSGH(XUO_SAVE_PACKET, "Enter World", 0x25, EnterWorld),
    /*0x1C*/ RMSGH(XUO_IGNORE_PACKET, "Server Talk", PACKET_VARIABLE_SIZE, Talk),
    /*0x1D*/ RMSGH(XUO_SAVE_PACKET, "Delete Object", 0x05, DeleteObject),
    /*0x1E*/ BMSG(XUO_SAVE_PACKET, "Animate?", 0x04),
    /*0x1F*/ BMSG(XUO_SAVE_PACKET, "Explode?", 0x08),
    /*0x20*/ RMSGH(XUO_SAVE_PACKET, "Update Player", 0x13, UpdatePlayer),
    /*0x21*/ RMSGH(XUO_IGNORE_PACKET, "Deny Walk", 0x08, DenyWalk),
    /*0x22*/ BMSGH(XUO_IGNORE_PACKET, "Confirm Walk", 0x03, ConfirmWalk),
    /*0x23*/ RMSGH(XUO_SAVE_PACKET, "Drag Animation", 0x1a, DragAnimation),
    /*0x24*/ RMSGH(XUO_SAVE_PACKET, "Open Container", 0x07, OpenContainer),
    /*0x25*/ RMSGH(XUO_SAVE_PACKET, "Update Contained Item", 0x14, UpdateContainedItem),
    /*0x26*/ BMSG(XUO_SAVE_PACKET, "Kick client (God client)", 0x05),
    /*0x27*/ RMSGH(XUO_SAVE_PACKET, "Deny Move Item", 0x02, DenyMoveItem),
    /*0x28*/ RMSGH(XUO_SAVE_PACKET, "End dragging item", 0x05, EndDraggingItem),
    /*0x29*/ RMSGH(XUO_SAVE_PACKET, "Drop Item Accepted", 0x01, DropItemAccepted),
    /*0x2A*/ RMSG(XUO_SAVE_PACKET, "Blood mode", 0x05),
    /*0x2B*/ BMSG(XUO_SAVE_PACKET, "Toggle God mode (God client)", 0x02),
    /*0x2C*/ BMSGH(XUO_IGNORE_PACKET, "Death Screen", 0x02, DeathScreen),
    /*0x2D*/ RMSGH(XUO_SAVE_PACKET, "Mobile Attributes", 0x11, MobileAttributes),
    /*0x2E*/ RMSGH(XUO_SAVE_PACKET, "Server Equip Item", 0x0f, EquipItem),
    /*0x2F*/ RMSG(XUO_SAVE_PACKET, "Combat Notification", 0x0a),
    /*0x30*/ RMSG(XUO_SAVE_PACKET, "Attack ok", 0x05),
    /*0x31*/ RMSG(XUO_SAVE_PACKET, "Attack end", 0x01),
    /*0x32*/ BMSG(XUO_SAVE_PACKET, "Toggle hack mover (God client)", 0x02),
    /*0x33*/ RMSGH(XUO_IGNORE_PACKET, "Pause Control", 0x02, PauseControl),
    /*0x34*/ SMSG(XUO_IGNORE_PACKET, "Status Request", 0x0a),
    /*0x35*/ BMSG(XUO_SAVE_PACKET, "Resource type (God client)", 0x28d),
    /*0x36*/ BMSG(XUO_SAVE_PACKET, "Resource tile data (God client)", PACKET_VARIABLE_SIZE),
    /*0x37*/ BMSG(XUO_SAVE_PACKET, "Move object (God client)", 0x08),
    /*0x38*/ RMSGH(XUO_SAVE_PACKET, "Pathfinding", 0x07, Pathfinding),
    /*0x39*/ BMSG(XUO_SAVE_PACKET, "Remove group (God client)", 0x09),
    /*0x3A*/ BMSGH(XUO_IGNORE_PACKET, "Update Skills", PACKET_VARIABLE_SIZE, UpdateSkills),
    /*0x3B*/ BMSGH(XUO_IGNORE_PACKET, "Vendor Buy Reply", PACKET_VARIABLE_SIZE, BuyReply),
    /*0x3C*/
    RMSGH(XUO_SAVE_PACKET, "Update Contained Items", PACKET_VARIABLE_SIZE, UpdateContainedItems),
    /*0x3D*/ BMSG(XUO_SAVE_PACKET, "Ship (God client)", 0x02),
    /*0x3E*/ BMSG(XUO_SAVE_PACKET, "Versions (God client)", 0x25),
    /*0x3F*/ BMSG(XUO_SAVE_PACKET, "Update Statics (God Client)", PACKET_VARIABLE_SIZE),
    /*0x40*/ BMSG(XUO_SAVE_PACKET, "Update terrains (God client)", 0xc9),
    /*0x41*/ BMSG(XUO_SAVE_PACKET, "Update terrains (God client)", PACKET_VARIABLE_SIZE),
    /*0x42*/ BMSG(XUO_SAVE_PACKET, "Update art (God client)", PACKET_VARIABLE_SIZE),
    /*0x43*/ BMSG(XUO_SAVE_PACKET, "Update animation (God client)", 0x229),
    /*0x44*/ BMSG(XUO_SAVE_PACKET, "Update hues (God client)", 0x2c9),
    /*0x45*/ BMSG(XUO_SAVE_PACKET, "Version OK (God client)", 0x05),
    /*0x46*/ BMSG(XUO_SAVE_PACKET, "New art (God client)", PACKET_VARIABLE_SIZE),
    /*0x47*/ BMSG(XUO_SAVE_PACKET, "New terrain (God client)", 0x0b),
    /*0x48*/ BMSG(XUO_SAVE_PACKET, "New animation (God client)", 0x49),
    /*0x49*/ BMSG(XUO_SAVE_PACKET, "New hues (God client)", 0x5d),
    /*0x4A*/ BMSG(XUO_SAVE_PACKET, "Destroy art (God client)", 0x05),
    /*0x4B*/ BMSG(XUO_SAVE_PACKET, "Check version (God client)", 0x09),
    /*0x4C*/ BMSG(XUO_SAVE_PACKET, "Script names (God client)", PACKET_VARIABLE_SIZE),
    /*0x4D*/ BMSG(XUO_SAVE_PACKET, "Edit script (God client)", PACKET_VARIABLE_SIZE),
    /*0x4E*/ RMSGH(XUO_IGNORE_PACKET, "Personal Light Level", 0x06, PersonalLightLevel),
    /*0x4F*/ RMSGH(XUO_IGNORE_PACKET, "Global Light Level", 0x02, LightLevel),
    /*0x50*/ BMSG(XUO_IGNORE_PACKET, "Board header", PACKET_VARIABLE_SIZE),
    /*0x51*/ BMSG(XUO_IGNORE_PACKET, "Board message", PACKET_VARIABLE_SIZE),
    /*0x52*/ BMSG(XUO_IGNORE_PACKET, "Post board message", PACKET_VARIABLE_SIZE),
    /*0x53*/ RMSGH(XUO_SAVE_PACKET, "Error Code", 0x02, ErrorCode),
    /*0x54*/ RMSGH(XUO_SAVE_PACKET, "Sound Effect", 0x0c, PlaySoundEffect),
    /*0x55*/ RMSGH(XUO_IGNORE_PACKET, "Login Complete", 0x01, LoginComplete),
    /*0x56*/ BMSGH(XUO_IGNORE_PACKET, "Map Data", 0x0b, MapData),
    /*0x57*/ BMSG(XUO_SAVE_PACKET, "Update regions (God client)", 0x6e),
    /*0x58*/ BMSG(XUO_SAVE_PACKET, "New region (God client)", 0x6a),
    /*0x59*/ BMSG(XUO_SAVE_PACKET, "New content FX (God client)", PACKET_VARIABLE_SIZE),
    /*0x5A*/ BMSG(XUO_SAVE_PACKET, "Update content FX (God client)", PACKET_VARIABLE_SIZE),
    /*0x5B*/ RMSGH(XUO_IGNORE_PACKET, "Set Time", 0x04, SetTime),
    /*0x5C*/ BMSG(XUO_SAVE_PACKET, "Restart Version", 0x02),
    /*0x5D*/ SMSG(XUO_IGNORE_PACKET, "Select Character", 0x49),
    /*0x5E*/ BMSG(XUO_SAVE_PACKET, "Server list (God client)", PACKET_VARIABLE_SIZE),
    /*0x5F*/ BMSG(XUO_SAVE_PACKET, "Add server (God client)", 0x31),
    /*0x60*/ BMSG(XUO_SAVE_PACKET, "Remove server (God client)", 0x05),
    /*0x61*/ BMSG(XUO_SAVE_PACKET, "Destroy static (God client)", 0x09),
    /*0x62*/ BMSG(XUO_SAVE_PACKET, "Move static (God client)", 0x0f),
    /*0x63*/ BMSG(XUO_SAVE_PACKET, "Area load (God client)", 0x0d),
    /*0x64*/ BMSG(XUO_SAVE_PACKET, "Area load request (God client)", 0x01),
    /*0x65*/ RMSGH(XUO_IGNORE_PACKET, "Set Weather", 0x04, SetWeather),
    /*0x66*/ BMSGH(XUO_IGNORE_PACKET, "Book Page Data", PACKET_VARIABLE_SIZE, BookData),
    /*0x67*/ BMSG(XUO_SAVE_PACKET, "Simped? (God client)", 0x15),
    /*0x68*/ BMSG(XUO_SAVE_PACKET, "Script attach (God client)", PACKET_VARIABLE_SIZE),
    /*0x69*/ BMSG(XUO_SAVE_PACKET, "Friends (God client)", PACKET_VARIABLE_SIZE),
    /*0x6A*/ BMSG(XUO_SAVE_PACKET, "Notify friend (God client)", 0x03),
    /*0x6B*/ BMSG(XUO_SAVE_PACKET, "Key use (God client)", 0x09),
    /*0x6C*/ BMSGH(XUO_IGNORE_PACKET, "Target Data", 0x13, Target),
    /*0x6D*/ RMSGH(XUO_SAVE_PACKET, "Play Music", 0x03, PlayMusic),
    /*0x6E*/ RMSGH(XUO_IGNORE_PACKET, "Character Animation", 0x0e, CharacterAnimation),
    /*0x6F*/ BMSGH(XUO_IGNORE_PACKET, "Secure Trading", PACKET_VARIABLE_SIZE, SecureTrading),
    /*0x70*/ RMSGH(XUO_IGNORE_PACKET, "Graphic Effect", 0x1c, GraphicEffect),
    /*0x71*/
    BMSGH(XUO_IGNORE_PACKET, "Bulletin Board Data", PACKET_VARIABLE_SIZE, BulletinBoardData),
    /*0x72*/ BMSGH(XUO_IGNORE_PACKET, "War Mode", 0x05, Warmode),
    /*0x73*/ BMSGH(XUO_IGNORE_PACKET, "Ping", 0x02, Ping),
    /*0x74*/ RMSGH(XUO_IGNORE_PACKET, "Vendor Buy List", PACKET_VARIABLE_SIZE, BuyList),
    /*0x75*/ SMSG(XUO_SAVE_PACKET, "Rename Character", 0x23),
    /*0x76*/ RMSG(XUO_SAVE_PACKET, "New Subserver", 0x10),
    /*0x77*/ RMSGH(XUO_SAVE_PACKET, "Update Character", 0x11, UpdateCharacter),
    /*0x78*/ RMSGH(XUO_SAVE_PACKET, "Update Object", PACKET_VARIABLE_SIZE, UpdateObject),
    /*0x79*/ BMSG(XUO_SAVE_PACKET, "Resource query (God client)", 0x09),
    /*0x7A*/ BMSG(XUO_SAVE_PACKET, "Resource data (God client)", PACKET_VARIABLE_SIZE),
    /*0x7B*/ RMSG(XUO_SAVE_PACKET, "Sequence?", 0x02),
    /*0x7C*/ RMSGH(XUO_IGNORE_PACKET, "Open Menu Gump", PACKET_VARIABLE_SIZE, OpenMenu),
    /*0x7D*/ SMSG(XUO_IGNORE_PACKET, "Menu Choice", 0x0d),
    /*0x7E*/ BMSG(XUO_SAVE_PACKET, "God view query (God client)", 0x02),
    /*0x7F*/ BMSG(XUO_SAVE_PACKET, "God view data (God client)", PACKET_VARIABLE_SIZE),
    /*0x80*/ SMSG(XUO_IGNORE_PACKET, "First Login", 0x3e),
    /*0x81*/ RMSG(XUO_SAVE_PACKET, "Change character", PACKET_VARIABLE_SIZE),
    /*0x82*/ RMSGH(XUO_IGNORE_PACKET, "Login Error", 0x02, LoginError),
    /*0x83*/ SMSG(XUO_IGNORE_PACKET, "Delete Character", 0x27),
    /*0x84*/ BMSG(XUO_SAVE_PACKET, "Change password", 0x45),
    /*0x85*/
    RMSGH(XUO_IGNORE_PACKET, "Character List Notification", 0x02, CharacterListNotification),
    /*0x86*/
    RMSGH(XUO_IGNORE_PACKET, "Resend Character List", PACKET_VARIABLE_SIZE, ResendCharacterList),
    /*0x87*/ BMSG(XUO_SAVE_PACKET, "Send resources (God client)", PACKET_VARIABLE_SIZE),
    /*0x88*/ RMSGH(XUO_IGNORE_PACKET, "Open Paperdoll", 0x42, OpenPaperdoll),
    /*0x89*/ RMSGH(XUO_SAVE_PACKET, "Corpse Equipment", PACKET_VARIABLE_SIZE, CorpseEquipment),
    /*0x8A*/ BMSG(XUO_SAVE_PACKET, "Trigger edit (God client)", PACKET_VARIABLE_SIZE),
    /*0x8B*/ RMSG(XUO_SAVE_PACKET, "Display sign gump", PACKET_VARIABLE_SIZE),
    /*0x8C*/ RMSGH(XUO_IGNORE_PACKET, "Relay Server", 0x0b, RelayServer),
    /*0x8D*/ SMSG(XUO_SAVE_PACKET, "UO3D create character", PACKET_VARIABLE_SIZE),
    /*0x8E*/ BMSG(XUO_SAVE_PACKET, "Move character (God client)", PACKET_VARIABLE_SIZE),
    /*0x8F*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0x90*/ RMSGH(XUO_IGNORE_PACKET, "Display Map", 0x13, DisplayMap),
    /*0x91*/ SMSG(XUO_IGNORE_PACKET, "Second Login", 0x41),
    /*0x92*/ BMSG(XUO_SAVE_PACKET, "Update multi data (God client)", PACKET_VARIABLE_SIZE),
    /*0x93*/ RMSGH(XUO_IGNORE_PACKET, "Open Book", 0x63, OpenBook),
    /*0x94*/ BMSG(XUO_SAVE_PACKET, "Update skills data (God client)", PACKET_VARIABLE_SIZE),
    /*0x95*/ BMSGH(XUO_IGNORE_PACKET, "Dye Data", 0x09, DyeData),
    /*0x96*/ BMSG(XUO_SAVE_PACKET, "Game central monitor (God client)", PACKET_VARIABLE_SIZE),
    /*0x97*/ RMSGH(XUO_SAVE_PACKET, "Move Player", 0x02, MovePlayer),
    /*0x98*/ BMSG(XUO_SAVE_PACKET, "All Names (3D Client Only)", PACKET_VARIABLE_SIZE),
    /*0x99*/ BMSGH(XUO_SAVE_PACKET, "Multi Placement", 0x1a, MultiPlacement),
    /*0x9A*/ BMSGH(XUO_SAVE_PACKET, "ASCII Prompt", PACKET_VARIABLE_SIZE, ASCIIPrompt),
    /*0x9B*/ SMSG(XUO_IGNORE_PACKET, "Help Request", 0x102),
    /*0x9C*/ BMSG(XUO_SAVE_PACKET, "Assistant request (God client)", 0x135),
    /*0x9D*/ BMSG(XUO_SAVE_PACKET, "GM single (God client)", 0x33),
    /*0x9E*/ RMSGH(XUO_IGNORE_PACKET, "Vendor Sell List", PACKET_VARIABLE_SIZE, SellList),
    /*0x9F*/ SMSG(XUO_IGNORE_PACKET, "Vendor Sell Reply", PACKET_VARIABLE_SIZE),
    /*0xA0*/ SMSG(XUO_IGNORE_PACKET, "Select Server", 0x03),
    /*0xA1*/ RMSGH(XUO_IGNORE_PACKET, "Update Hitpoints", 0x09, UpdateHitpoints),
    /*0xA2*/ RMSGH(XUO_IGNORE_PACKET, "Update Mana", 0x09, UpdateMana),
    /*0xA3*/ RMSGH(XUO_IGNORE_PACKET, "Update Stamina", 0x09, UpdateStamina),
    /*0xA4*/ SMSG(XUO_SAVE_PACKET, "System Information", 0x95),
    /*0xA5*/ RMSGH(XUO_SAVE_PACKET, "Open URL", PACKET_VARIABLE_SIZE, OpenUrl),
    /*0xA6*/ RMSGH(XUO_IGNORE_PACKET, "Tip Window", PACKET_VARIABLE_SIZE, TipWindow),
    /*0xA7*/ SMSG(XUO_SAVE_PACKET, "Request Tip", 0x04),
    /*0xA8*/ RMSGH(XUO_IGNORE_PACKET, "Server List", PACKET_VARIABLE_SIZE, ServerList),
    /*0xA9*/ RMSGH(XUO_IGNORE_PACKET, "Character List", PACKET_VARIABLE_SIZE, CharacterList),
    /*0xAA*/ RMSGH(XUO_IGNORE_PACKET, "Attack Reply", 0x05, AttackCharacter),
    /*0xAB*/ RMSGH(XUO_SAVE_PACKET, "Text Entry Dialog", PACKET_VARIABLE_SIZE, TextEntryDialog),
    /*0xAC*/ SMSG(XUO_SAVE_PACKET, "Text Entry Dialog Reply", PACKET_VARIABLE_SIZE),
    /*0xAD*/ SMSG(XUO_IGNORE_PACKET, "Unicode Client Talk", PACKET_VARIABLE_SIZE),
    /*0xAE*/ RMSGH(XUO_IGNORE_PACKET, "Unicode Server Talk", PACKET_VARIABLE_SIZE, UnicodeTalk),
    /*0xAF*/ RMSGH(XUO_SAVE_PACKET, "Display Death", 0x0d, DisplayDeath),
    /*0xB0*/ RMSGH(XUO_IGNORE_PACKET, "Open Gump", PACKET_VARIABLE_SIZE, OpenGump),
    /*0xB1*/ SMSG(XUO_IGNORE_PACKET, "Gump Choice", PACKET_VARIABLE_SIZE),
    /*0xB2*/ BMSG(XUO_SAVE_PACKET, "Chat Data", PACKET_VARIABLE_SIZE),
    /*0xB3*/ RMSG(XUO_SAVE_PACKET, "Chat Text ?", PACKET_VARIABLE_SIZE),
    /*0xB4*/ BMSG(XUO_SAVE_PACKET, "Target object list (God client)", PACKET_VARIABLE_SIZE),
    /*0xB5*/ BMSGH(XUO_SAVE_PACKET, "Open Chat Window", 0x40, OpenChat),
    /*0xB6*/ SMSG(XUO_SAVE_PACKET, "Popup Help Request", 0x09),
    /*0xB7*/ RMSG(XUO_SAVE_PACKET, "Popup Help Data", PACKET_VARIABLE_SIZE),
    /*0xB8*/
    BMSGH(XUO_IGNORE_PACKET, "Character Profile", PACKET_VARIABLE_SIZE, CharacterProfile),
    /*0xB9*/ RMSGH(XUO_SAVE_PACKET, "Enable locked client features", 0x03, EnableLockedFeatures),
    /*0xBA*/ RMSGH(XUO_IGNORE_PACKET, "Display Quest Arrow", 0x06, DisplayQuestArrow),
    /*0xBB*/ SMSG(XUO_SAVE_PACKET, "Account ID ?", 0x09),
    /*0xBC*/ RMSGH(XUO_IGNORE_PACKET, "Season", 0x03, Season),
    /*0xBD*/ BMSGH(XUO_SAVE_PACKET, "Client Version", PACKET_VARIABLE_SIZE, ClientVersion),
    /*0xBE*/ BMSGH(XUO_SAVE_PACKET, "Assist Version", PACKET_VARIABLE_SIZE, AssistVersion),
    /*0xBF*/ BMSGH(XUO_SAVE_PACKET, "Extended Command", PACKET_VARIABLE_SIZE, ExtendedCommand),
    /*0xC0*/ RMSGH(XUO_IGNORE_PACKET, "Graphical Effect", 0x24, GraphicEffect),
    /*0xC1*/
    RMSGH(XUO_IGNORE_PACKET, "Display cliloc String", PACKET_VARIABLE_SIZE, DisplayClilocString),
    /*0xC2*/ BMSGH(XUO_SAVE_PACKET, "Unicode prompt", PACKET_VARIABLE_SIZE, UnicodePrompt),
    /*0xC3*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xC4*/ UMSG(XUO_SAVE_PACKET, 0x06),
    /*0xC5*/ BMSG(XUO_SAVE_PACKET, "Invalid map (God client)", 0xcb),
    /*0xC6*/ RMSG(XUO_SAVE_PACKET, "Invalid map enable", 0x01),
    /*0xC7*/ RMSGH(XUO_IGNORE_PACKET, "Graphical Effect", 0x31, GraphicEffect),
    /*0xC8*/ BMSGH(XUO_SAVE_PACKET, "Client View Range", 0x02, ClientViewRange),
    /*0xC9*/ BMSG(XUO_SAVE_PACKET, "Trip time", 0x06),
    /*0xCA*/ BMSG(XUO_SAVE_PACKET, "UTrip time", 0x06),
    ...
    /*0xD0*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xD1*/ RMSGH(XUO_IGNORE_PACKET, "Logout", 0x02, Logout),
    /*0xD2*/ RMSGH(XUO_SAVE_PACKET, "Update Character (New)", 0x19, UpdateCharacter),
    /*0xD3*/ RMSGH(XUO_SAVE_PACKET, "Update Object (New)", PACKET_VARIABLE_SIZE, UpdateObject),
    /*0xD4*/ BMSGH(XUO_IGNORE_PACKET, "Open Book (New)", PACKET_VARIABLE_SIZE, OpenBookNew),
    /*0xD5*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xD6*/ BMSGH(XUO_IGNORE_PACKET, "Mega cliloc", PACKET_VARIABLE_SIZE, MegaCliloc),
    /*0xD7*/ SMSG(XUO_SAVE_PACKET, "+AoS command", PACKET_VARIABLE_SIZE),
    /*0xD8*/ RMSGH(XUO_IGNORE_PACKET, "Custom house", PACKET_VARIABLE_SIZE, CustomHouse),
    /*0xD9*/ SMSG(XUO_SAVE_PACKET, "+Metrics", 0x10c),
    /*0xDA*/ BMSG(XUO_SAVE_PACKET, "Mahjong game command", PACKET_VARIABLE_SIZE),
    /*0xDB*/ RMSG(XUO_SAVE_PACKET, "Character transfer log", PACKET_VARIABLE_SIZE),
    /*0xDC*/ RMSGH(XUO_IGNORE_PACKET, "OPL Info Packet", 9, OPLInfo),
    /*0xDD*/
    RMSGH(XUO_IGNORE_PACKET, "Compressed Gump", PACKET_VARIABLE_SIZE, OpenCompressedGump),
    /*0xDE*/ RMSG(XUO_SAVE_PACKET, "Update characters combatants", PACKET_VARIABLE_SIZE),
    /*0xDF*/ RMSGH(XUO_SAVE_PACKET, "Buff/Debuff", PACKET_VARIABLE_SIZE, BuffDebuff),
    /*0xE0*/ SMSG(XUO_SAVE_PACKET, "Bug Report KR", PACKET_VARIABLE_SIZE),
    /*0xE1*/ SMSG(XUO_SAVE_PACKET, "Client Type KR/SA", 0x09),
    /*0xE2*/ RMSGH(XUO_IGNORE_PACKET, "New Character Animation", 0xa, NewCharacterAnimation),
    /*0xE3*/ RMSG(XUO_SAVE_PACKET, "KR Encryption Responce", 0x4d),
    /*0xE4*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xE5*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xE6*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xE7*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xE8*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xE9*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xEA*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xEB*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xEC*/ SMSG(XUO_SAVE_PACKET, "Equip Macro", PACKET_VARIABLE_SIZE),
    /*0xED*/ SMSG(XUO_SAVE_PACKET, "Unequip item macro", PACKET_VARIABLE_SIZE),
    /*0xEE*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xEF*/ SMSG(XUO_SAVE_PACKET, "KR/2D Client Login/Seed", 0x15),
    /*0xF0*/
    BMSGH(XUO_SAVE_PACKET, "Krrios client special", PACKET_VARIABLE_SIZE, KrriosClientSpecial),
    /*0xF1*/
    SMSG(XUO_SAVE_PACKET, "Client-Server Time Synchronization Request", PACKET_VARIABLE_SIZE),
    /*0xF2*/
    RMSG(XUO_SAVE_PACKET, "Client-Server Time Synchronization Response", PACKET_VARIABLE_SIZE),
    /*0xF3*/ RMSGH(XUO_SAVE_PACKET, "Update Item (SA)", 0x18, UpdateItemSA),
    /*0xF4*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xF5*/ RMSGH(XUO_IGNORE_PACKET, "Display New Map", 0x15, DisplayMap),
    /*0xF6*/ RMSGH(XUO_SAVE_PACKET, "Boat moving", PACKET_VARIABLE_SIZE, BoatMoving),
    /*0xF7*/ RMSGH(XUO_SAVE_PACKET, "Packets (0xF3) list", PACKET_VARIABLE_SIZE, PacketsList),
    /*0xF8*/ SMSG(XUO_SAVE_PACKET, "Character Creation (7.0.16.0)", 0x6a),
    /*0xF9*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xFA*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xFB*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xFC*/ BMSGH(XUO_SAVE_PACKET, "CrossUO messages", PACKET_VARIABLE_SIZE, CrossMessages),
    /*0xFD*/ UMSG(XUO_SAVE_PACKET, PACKET_VARIABLE_SIZE),
    /*0xFE*/ RMSG(XUO_SAVE_PACKET, "Razor Handshake", 0x8),
...
#if CV_PRINT != 0
#define CVPRINT(s) TRACE(Netwrok, s)
#else //CV_PRINT==0
#define CVPRINT(s)
#endif //CV_PRINT!=0
...
void CPacketManager::ConfigureClientVersion(uint32_t newClientVersion)
{
    ...
    {
        CVPRINT("Set standard length for packet 0x0B (< 5.0.0a)");
        m_Packets[0x0B].Size = 0x10A;
        CVPRINT("Set standard length for packet 0x16 (< 5.0.0a)");
        m_Packets[0x16].Size = 0x01;
        CVPRINT("Set standard length for packet 0x31 (< 5.0.0a)");
    ...
    {
        CVPRINT("Set standard length for packet 0xE1 (<= 5.0.9.0)");
    ...
    {
        CVPRINT("Set standard length for packet 0xE3 (<= 6.0.1.3)");
        m_Packets[0xE3].Size = 0x4D;
        CVPRINT("Set standard length for packet 0xE6 (<= 6.0.1.3)");
        m_Packets[0xE6].Size = PACKET_VARIABLE_SIZE;
        CVPRINT("Set standard length for packet 0xE7 (<= 6.0.1.3)");
        m_Packets[0xE7].Size = PACKET_VARIABLE_SIZE;
        CVPRINT("Set standard length for packet 0xE8 (<= 6.0.1.3)");
        m_Packets[0xE8].Size = PACKET_VARIABLE_SIZE;
        CVPRINT("Set standard length for packet 0xE9 (<= 6.0.1.3)");
        m_Packets[0xE9].Size = PACKET_VARIABLE_SIZE;
        CVPRINT("Set standard length for packet 0xEA (<= 6.0.1.3)");
    ...
    {
        CVPRINT("Set standard length for packet 0x08 (<= 6.0.1.7)");
        m_Packets[0x08].Size = 0x0E;
        CVPRINT("Set standard length for packet 0x25 (<= 6.0.1.7)");
    ...
    {
        CVPRINT("Set standard length for packet 0xEE (<= 6.0.6.0)");
        m_Packets[0xEE].Size = PACKET_VARIABLE_SIZE;
        CVPRINT("Set standard length for packet 0xEF (<= 6.0.6.0)");
        m_Packets[0xEF].Size = 0x15;
        CVPRINT("Set standard length for packet 0xF1 (<= 6.0.6.0)");
    ...
    {
        CVPRINT("Set standard length for packet 0xB9 (<= 6.0.14.2)");
    ...
    {
        CVPRINT("Set standard length for packet 0xEE (<= 7.0.0.0)");
        m_Packets[0xEE].Size = PACKET_VARIABLE_SIZE;
        CVPRINT("Set standard length for packet 0xEF (<= 7.0.0.0)");
        m_Packets[0xEF].Size = 0x15;
        /*CVPRINT("Set standard length for packet 0xF0 (<= 7.0.0.0)");
        m_Packets[0xF0].size = PACKET_VARIABLE_SIZE;
        CVPRINT("Set standard length for packet 0xF1 (<= 7.0.0.0)");
        m_Packets[0xF1].size = PACKET_VARIABLE_SIZE;
        CVPRINT("Set standard length for packet 0xF2 (<= 7.0.0.0)");
    ...
    {
        CVPRINT("Set standard length for packet 0x24 (<= 7.0.9.0)");
        m_Packets[0x24].Size = 0x07;
        CVPRINT("Set standard length for packet 0x99 (<= 7.0.9.0)");
        m_Packets[0x99].Size = 0x1A;
        CVPRINT("Set standard length for packet 0xBA (<= 7.0.9.0)");
        m_Packets[0xBA].Size = 0x06;
        CVPRINT("Set standard length for packet 0xF3 (<= 7.0.9.0)");
        m_Packets[0xF3].Size = 0x18;

        // Already changed in client 7.0.8.2
        CVPRINT("Set standard length for packet 0xF1 (<= 7.0.9.0)");
        m_Packets[0xF1].Size = PACKET_VARIABLE_SIZE;
        CVPRINT("Set standard length for packet 0xF2 (<= 7.0.9.0)");
        m_Packets[0xF2].Size = PACKET_VARIABLE_SIZE;
    }
    ...
    {
        CVPRINT("Set standard length for packet 0x24 (<= 7.0.18.0)");
...
void CPacketManager::OnPacket()
{
    ...
    if (info.save)
    {
#if defined(XUO_WINDOWS) // FIXME: localtime_s (use C++ if possible)
        time_t rawtime;
        struct tm timeinfo;
        char buffer[80];
    ...
    {
        WARN(Network, "message direction invalid: 0x{:0>2x}", *Start);
    }
...
void CPacketManager::PluginReceiveHandler(uint8_t *buf, int size)
{
    ...
    INFO(
        ...
        g_TotalRecvSize,
        info.Name);
    INFO_DUMP(Plugin, "RECV:", Start, (int)Size);

    g_LastPacketTime = ticks;
    if (info.Direction != DIR_RECV && info.Direction != DIR_BOTH)
    {
        WARN(Plugin, "message direction invalid: 0x{:0>2x}", *buf);
    }
...
PACKET_HANDLER(ResendCharacterList)
{
    ...
    if (*Start == 0x86)
    {
        INFO(Network, "{:=^30}", "resend chars");
    }
    else
    {
        INFO(Network, "{:=^30}", "chars");
    }
    ...
    if (numSlots == 0)
    {
        WARN(Network, "no slots in character list");
    }
    else
    {
        int selectedPos = -1;
        for (int i = 0; i < numSlots; i++)
        {
            ...
            INFO(Network, "{}: {} ({})", i, name, name.length());
        }
...
PACKET_HANDLER(EnterWorld)
{
    ...
    if (g_World != nullptr)
    {
        WARN(Network, "duplicate enter world message");
    ...
    INFO(Network, "player 0x{:0>8x} entered the world", serial);
...
PACKET_HANDLER(UpdatePlayer)
{
    ...
    INFO(
        Network,
        "0x{:0>8x} 0x{:0>4x} {} 0x{:0>4x} 0x{:0>2x} {} {} {} {} {}",
        serial,
        graphic,
        graphicIncrement,
        color,
...
PACKET_HANDLER(UpdateContainedItems)
{
    ...
    for (int i = 0; i < itemsCount; i++)
    {
        ...
        if (i == 0)
        {
            ...
            {
                INFO(Network, "making {:0>8x} empty...", containerSerial);
...
PACKET_HANDLER(UpdateCharacter)
{
    ...
    {
        direction &= 0x87;
        direction |= 0x80; // set bad orionuo running bit
        // FIXME: rework walkdata/pathfind
    }
    ...
    else if (gumpid == 0x0030) //Buylist
    {
        ...
        if (vendor != nullptr)
        {
            ...
            for (int layer = OL_BUY_RESTOCK; layer < OL_BUY + 1; layer++)
            {
                ...
                if (item == nullptr)
                {
                    WARN(Network, "buy layer {} not found", layer);
                    continue;
                }
                ...
                if (item == nullptr)
                {
                    WARN(Network, "buy item not found");
                    continue;
                }
        ...
        {
            WARN(Network, "buy vendor not found");
        }
    }
    else //Container
    {
...
PACKET_HANDLER(UpdateSkills)
{
    ...
    INFO(Network, "skill update type {} (cap={})", type, haveCap);
    ...
    while (!IsEOF())
    {
        ...
        if (id < g_SkillsManager.Count && skill != nullptr)
        {
            ...
            if (haveCap)
            {
                DEBUG(Network, "skill {} is {}|{}|{}", id, baseVal, realVal, cap);
            }
            else
            {
                DEBUG(Network, "skill {} is {}|{}", id, baseVal, realVal);
            }
        }
        else
        {
            DEBUG(Network, "unknown skill update {}", id);
        }
...
PACKET_HANDLER(ExtendedCommand)
{
    ...
    switch (cmd)
    {
        ...
        case 0x1B: //New spellbook content
        {
            ...
            {
                WARN(Network, "spellbook not found");
                return;
            }
        ...
        case 0x1D:
        {
            ...
            INFO(Network, "searching house: 0x{:0>8x} 0x{:0>8x}", serial, revision);
            if (house != nullptr)
            {
                INFO(Network, "found house: 0x{:0>8x} 0x{:0>8x}", house->Serial, house->Revision);
            }
...
PACKET_HANDLER(UnicodeTalk)
{
    ...
    INFO(Network, "{} said: {}", name, ToString(str));
...
PACKET_HANDLER(PlayMusic)
{
    ...
    INFO(Network, "play music 0x{:0>4x}", index);
...
PACKET_HANDLER(MegaCliloc)
{
    ...
    while (Ptr < end)
    {
        ...
        DEBUG(Network, "cliloc: argstr={}", ToString(str));
        DEBUG(Network, "cliloc: 0x{:0>8x} len={} arg={}", cliloc, len, ToString(argument));
...
PACKET_HANDLER(OpenMenu)
{
    ...
    {
        int x = (g_GameWindow.GetSize().Width / 2) - 200;
        int y = (g_GameWindow.GetSize().Height / 2) - ((121 + (count * 21)) / 2);
...
PACKET_HANDLER(OpenCompressedGump)
{
    ...
    if (cLen < 1)
    {
        WARN(Network, "bad compressed gumpdata, len={}", cLen);
        return;
    }
    if ((int)(28 + cLen) > Size)
    {
        WARN(Network, "bad compressed gumpdata");
        return;
    }

    // Layout data.....
    vector<uint8_t> decLayoutData(dLen);
    INFO(Network, "gump layout:");
    INFO(Network, "\tSenderID=0x{:0>8x}", senderID);
    INFO(Network, "\tGumpID=0x{:0>8x}", gumpID);
    INFO(Network, "\tCLen={}", cLen);
    INFO(Network, "\tDLen={}", dLen);
    INFO(Network, "decompressing layout gump data...");
    int z_err = mz_uncompress(&decLayoutData[0], &dLen, Ptr, cLen);
    if (z_err != Z_OK)
    {
        WARN(Network, "gump layout decompression error: {}", z_err);
        return;
    }

    INFO(Network, "gump layout data decompressed");
    // Text data.....
    ...
    if (linesCount > 0)
    {
        ...
        if (z_err != Z_OK)
        {
            WARN(Network, "text gump decompression error: {}", z_err);
            return;
        }

        INFO(Network, "text gump data decompressed, text lines:");
        INFO(Network, "\tLinesCount={}", linesCount);
        INFO(Network, "\tCTLen={}", cTLen);
        INFO(Network, "\tDTLen={}", dTLen);
    }
    ...
    INFO(Network, "gump decompressed. newsize={}", newsize);
    Size = newsize;
    Start = newbuf;
    End = Start + Size;
...
PACKET_HANDLER(DisplayMap)
{
    ...
    TRACE(Network, "gumpid = 0x{:0>4x}", gumpid);
    TRACE(Network, "gumpx={} gumpy={}", startX, startY);
    TRACE(Network, "gumptx={} gumpty={}", endX, endY);
    TRACE(Network, "gumpw={} gumph={}", width, height);
...
PACKET_HANDLER(BuyList)
{
    ...
    {
        WARN(Network, "unknown layer for buy container");
    }
}
...
PACKET_HANDLER(CustomHouse)
{
    ...
    INFO(Network, "house update in cache: 0x{:0>8x} 0x{:0>8x}", serial, revision);
    house->m_Items.clear();
    ...
    for (int plane = 0; plane < planes; plane++)
    {
        ...
        if (z_err != Z_OK)
        {
            WARN(
                Network,
                "bad CustomHouseStruct compressed data received from server, house serial: {}",
                serial);
            continue;
        }
...
PACKET_HANDLER(CrossMessages)
{
    ...
    switch (command)
    {
        case OCT_XUO_FEATURES:
        {
            g_GameFeaturesFlags = ReadUInt32BE();
        ...
        case OCT_PLAY_MACRO:
        {
            ...
            for (int m = 0; m < count; m++)
            {
                ...
                {
                    WARN(Plugin, "invalid macro name: {}", name);
                    continue;
                }
...
PACKET_HANDLER(PacketsList)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        {
            WARN(Network, "unknown packet ID=0x{:0>2X} in packet 0xF7", id);
            break;
        }
    }
}
