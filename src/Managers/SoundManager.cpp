...
SoundHandle CSoundManager::LoadSoundEffect(CIndexSound &is)
{
    ...
    {
        ERROR(Client, "sound: creating voice: {}", GetErrorDescription());
        free(is.m_WaveFile);
...
void CSoundManager::PlaySoundEffect(SoundHandle stream, float volume)
{
    ...
    {
        ERROR(Client, "sound: trying to play unallocated sound.");
    }
}
...
void CSoundManager::FreeSound(SoundHandle stream)
{
    ...
    {
        ERROR(Client, "sound: trying to free unallocated sound");
    }
    stream = SOUND_NULL;
}
...
void CSoundManager::PlayMidi(int index, bool warmode)
{
    ...
    {
        ERROR(Client, "sound: music id is out of range: {}", index);
        return;
    }
