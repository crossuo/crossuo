    ...
    uint8_t font, const astr_t &str, int pos, int width, TEXT_ALIGN_TYPE align, uint16_t flags)
{
    ...
    auto info = GetInfoA(font, str, align, flags, width);
    if (info == nullptr)
    {
        return p;
    }
...
int CFontsManager::CalculateCaretPosA(
    uint8_t font, const astr_t &str, int x, int y, int width, TEXT_ALIGN_TYPE align, uint16_t flags)
{
...
int CFontsManager::GetWidthA(uint8_t font, const astr_t &str)
{
    ...
    uint8_t font, const astr_t &str, int maxWidth, TEXT_ALIGN_TYPE align, uint16_t flags)
{
    ...
    auto info = GetInfoA(font, str, align, flags, maxWidth);
    int textWidth = 0;
    while (info != nullptr)
    {
...
astr_t CFontsManager::GetTextByWidthA(uint8_t font, const astr_t &str, int width, bool isCropped)
{
    if (font >= FontCount || str.empty())
    {
        return astr_t("");
    ...
    uint16_t flags)
{
    if (((flags & UOFONT_FIXED) != 0) || ((flags & UOFONT_CROPPED) != 0))
    {
        ...
        if (realWidth > width)
        {
            auto newstr = GetTextByWidthA(font, str, width, (flags & UOFONT_CROPPED) != 0);
...
bool CFontsManager::GenerateABase(
    ...
    uint16_t flags)
{
    auto pixels = GeneratePixelsA(font, th, str, color, width, align, flags);
    bool result = false;
    if (!pixels.empty())
    {
    ...
    uint8_t font, const wstr_t &str, int pos, int width, TEXT_ALIGN_TYPE align, uint16_t flags)
{
    ...
    auto info = GetInfoW(font, str, align, flags, width);
    if (info == nullptr)
    {
        return p;
    }
    ...
    while (info != nullptr)
    {
        ...
        if (pos <= info->CharStart + len && (int)info->Data.size() >= len)
        {
            ...
            {
                //collect data about width of each character
                const auto &ch = info->Data[i].item;
...
int CFontsManager::CalculateCaretPosW(
    uint8_t font, const wstr_t &str, int x, int y, int width, TEXT_ALIGN_TYPE align, uint16_t flags)
{
...
int CFontsManager::GetWidthW(uint8_t font, const wstr_t &str)
{
    ...
    uint8_t font, const wstr_t &str, int maxWidth, TEXT_ALIGN_TYPE align, uint16_t flags)
{
    ...
    auto info = GetInfoW(font, str, align, flags, maxWidth);
    int textWidth = 0;
    while (info != nullptr)
    {
        if (info->Width > textWidth)
...
int CFontsManager::GetHeightW(
    uint8_t font, const wstr_t &str, int width, TEXT_ALIGN_TYPE align, uint16_t flags)
{
    if (font >= 20 || (m_UnicodeFontAddress[font] == 0u) || str.empty())
    {
        return 0;
...
wstr_t CFontsManager::GetTextByWidthW(uint8_t font, const wstr_t &str, int width, bool isCropped)
{
    if (font >= 20 || (m_UnicodeFontAddress[font] == 0u) || str.empty())
    {
        return wstr_t({});
...
uint16_t CFontsManager::GetWebLinkID(const astr_t &link, uint32_t &color)
{
    uint16_t linkID = 0;
...
CFontsManager::GetHTMLData(uint8_t font, const wstr_t &str, TEXT_ALIGN_TYPE align, uint16_t flags)
{
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        if (si == L'<')
        {
            ...
            auto tag = ParseHTMLTag(str, i, endTag, newInfo);
            if (tag == HTT_NONE)
            {
                continue;
            }
        ...
        if (si != 0)
        {
            data.push_back({ char16_t(si), ci.Font, ci.Align, ci.Flags, ci.Color, ci.Link });
            newlen++;
        }
    }
    return data;
...
void CFontsManager::TrimHTMLString(astr_t &str)
{
...
uint32_t CFontsManager::GetHTMLColorFromText(astr_t &str1)
{
    uint32_t color = 0;
...
void CFontsManager::GetHTMLInfoFromContent(HTML_DATA_INFO &info, const astr_t &content)
{
    ...
    auto strings = parser.GetTokens(content.c_str());
    ...
    for (int i = 0; i < size; i += 2)
    {
        ...
        switch (info.Tag)
        {
            case HTT_BODY:
            {
                ...
                {
                    m_LeftMargin = str_to_int(value);
                }
                else if (str == "topmargin")
                {
                    m_TopMargin = str_to_int(value);
                }
                else if (str == "rightmargin")
                {
                    m_RightMargin = str_to_int(value);
                }
                else if (str == "bottommargin")
                {
                    m_BottomMargin = str_to_int(value);
                }
...
HTML_TAG_TYPE
CFontsManager::ParseHTMLTag(const wstr_t &str, int &i, bool &endTag, HTML_DATA_INFO &info)
{
...
PMULTILINES_FONT_INFO CFontsManager::GetInfoHTML(
    uint8_t font, const wstr_t &str, TEXT_ALIGN_TYPE align, uint16_t flags, int width)
{
    HTMLCHAR_LIST htmlData = GetHTMLData(font, str, align, flags);
    if (htmlData.empty())
    {
        return nullptr;
    }
    ...
    uint8_t font, const wstr_t &str, TEXT_ALIGN_TYPE align, uint16_t flags, int width)
{
    ...
    if (m_UseHTML)
    {
        return GetInfoHTML(font, str, align, flags, width);
    }
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        if (ptr->Width + readWidth + ((char)data[0] + (char)data[2]) > width || si == L'\n')
        {
            ...
            else
            {
                if (isFixed)
                {
                    MULTILINES_FONT_DATA mfd = { char16_t(si),      current_flags, current_font, 0,
                                                 current_charcolor, nullptr };
                    ptr->Data.push_back(mfd);
    ...
    uint16_t flags)
{
    if (((flags & UOFONT_FIXED) != 0) || ((flags & UOFONT_CROPPED) != 0))
    {
        ...
        if (realWidth > width)
        {
            auto newstr = GetTextByWidthW(font, str, width, (flags & UOFONT_CROPPED) != 0);
    ...
    uint16_t flags)
{
    ...
    if (m_UseHTML && ((m_LeftMargin != 0) || (m_RightMargin != 0)))
    {
        ...
        info = GetInfoW(font, str, align, flags, newWidth);
        if (info == nullptr)
        {
            return pData;
        }
    ...
    while (ptr != nullptr)
    {
        ...
        uint16_t oldLink = 0;
        const int dataSize = (int)ptr->Data.size();
        for (int i = 0; i < dataSize; i++)
        {
            const MULTILINES_FONT_DATA &dataPtr = ptr->Data[i];
            const auto &si = dataPtr.item;
...
bool CFontsManager::GenerateWBase(
    ...
    uint16_t flags)
{
    auto pixels = GeneratePixelsW(font, th, str, color, cell, width, align, flags);
    bool result = false;
    if (!pixels.empty())
    {
        th.Init(th.Width, th.Height, &pixels[0], false);
