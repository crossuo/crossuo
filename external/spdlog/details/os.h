...
inline void prevent_child_fd(FILE *f)
{
    ...
    if (!::SetHandleInformation(file_handle, HANDLE_FLAG_INHERIT, 0))
        fprintf(stderr, "SetHandleInformation failed %d", errno);
#endif
#else
    auto fd = fileno(f);
    if (fcntl(fd, F_SETFD, FD_CLOEXEC) == -1)
    {
        fprintf(stderr, "fcntl with FD_CLOEXEC failed %d", errno);
    }
#endif
}
...
inline size_t filesize(FILE *f)
{
    if (f == nullptr)
    {
        fprintf(stderr, "Failed getting file size. fd is null");
    }
...
#endif
#endif
    fprintf(stderr, "Failed getting file size from fd %d", errno);
    return 0;
}
...
inline int utc_minutes_offset(const std::tm &tm = details::os::localtime())
{
    ...
    if (rv == TIME_ZONE_ID_INVALID)
        fprintf(stdout, "Failed getting timezone info. %d", errno);
