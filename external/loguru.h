...
# Version history
	...
	* CrossUO - 2019-02-11 - Merge back into one header file only
		- replace thread_name with system/module name
		- level 1 = debug
		- level 9 = trace
...
# Usage
	...
	int main(int argc, char* argv[]) {
		...
		LOG_F(0, INFO, "The magic number is %d", 42);
	}
...
#define VTSEQ(ID) ("\x1b[" #ID "m")
	...
	void file_reopen(void* user_data)
	{
		...
		if (!file_abs->fp || (ret = stat(file_abs->path, &st)) == -1 || (st.st_ino != file_abs->st.st_ino)) {
			...
			if (!file_abs->fp) {
				LOG_F(0, INFO, "Reopening file '%s' due to previous error", file_abs->path);
			}
			else if (ret < 0) {
				const auto why = errno_as_text();
				LOG_F(0, INFO, "Reopening file '%s' due to '%s'", file_abs->path, why.c_str());
			} else {
				LOG_F(0, INFO, "Reopening file '%s' due to file changed", file_abs->path);
			}
			// try reopen current file.
			if (!create_directories(file_abs->path)) {
				LOG_F(0, ERROR, "Failed to create directories to '%s'", file_abs->path);
			}
			file_abs->fp = fopen(file_abs->path, file_abs->mode_str);
			if (!file_abs->fp) {
				LOG_F(0, ERROR, "Failed to open '%s'", file_abs->path);
			} else {
	...
	void init(int& argc, char* argv[], const char* verbosity_flag)
	{
		...
		if (!getcwd(s_current_dir, sizeof(s_current_dir)))
		{
			...
			LOG_F(0, WARNING, "Failed to get current working directory: %s", error_text.c_str());
		}
		...
		LOG_F(0, INFO, "arguments: %s", s_arguments.c_str());
		if (strlen(s_current_dir) != 0)
		{
			LOG_F(0, INFO, "Current dir: %s", s_current_dir);
		}
		LOG_F(0, INFO, "stderr verbosity: %d", g_stderr_verbosity);
		LOG_F(0, INFO, "-----------------------------------");
	...
	#endif
				if (errno != EEXIST) {
					LOG_F(0, ERROR, "Failed to create directory '%s'", file_path);
					LOG_IF_F(0, ERROR, errno == EACCES,       "EACCES");
					LOG_IF_F(0, ERROR, errno == ENAMETOOLONG, "ENAMETOOLONG");
					LOG_IF_F(0, ERROR, errno == ENOENT,       "ENOENT");
					LOG_IF_F(0, ERROR, errno == ENOTDIR,      "ENOTDIR");
					LOG_IF_F(0, ERROR, errno == ELOOP,        "ELOOP");
	...
	bool add_file(const char* path_in, FileMode mode, Verbosity verbosity)
	{
		...
		if (!create_directories(path)) {
			LOG_F(0, ERROR, "Failed to create directories to '%s'", path);
		}

		const char* mode_str = (mode == FileMode::Truncate ? "w" : "a");
		auto file = fopen(path, mode_str);
		if (!file) {
			LOG_F(0, ERROR, "Failed to open '%s'", path);
			return false;
		}
#if LOGURU_WITH_FILEABS
		...
		LOG_F(0, INFO, "Logging to '%s', mode: '%s', verbosity: %d", path, mode_str, verbosity);
		return true;
	}
	...
	void add_stack_cleanup(const char* find_this, const char* replace_with_this)
	{
		if (strlen(find_this) <= strlen(replace_with_this)) {
			LOG_F(0, WARNING, "add_stack_cleanup: the replacement should be shorter than the pattern!");
			return;
		}
	...
	Verbosity get_verbosity_from_name(const char* name)
	{
		...
		if (verbosity == Verbosity_INVALID) {
			...
			} else if (strcmp(name, "DEBUG") == 0) { // crossuo
				verbosity = Verbosity_1;
			} else if (strcmp(name, "TRACE") == 0) { // crossuo
	...
	bool remove_callback(const char* id)
	{
		...
		if (it != s_callbacks.end()) {
			...
			LOG_F(0,ERROR, "Failed to locate callback with id '%s'", id);
			return false;
		}
	}
	...
	static void print_preamble_header(char* out_buff, size_t out_buff_size)
	{
		...
		if (g_preamble_thread && pos < out_buff_size) {
			pos += snprintf(out_buff + pos, out_buff_size - pos, "[%-*s]", LOGURU_THREADNAME_WIDTH, " module");
		}
	...
	static void print_preamble(char* out_buff, size_t out_buff_size, int sys, Verbosity verbosity, const char* file, unsigned line)
	{
		if (out_buff_size == 0) { return; }
		out_buff[0] = '\0';
		...
		if (g_preamble_verbose && pos < out_buff_size) {
			pos += snprintf(out_buff + pos, out_buff_size - pos, "|%5s",
			               level_buff);
		}
		if (g_preamble_pipe && pos < out_buff_size) {
			pos += snprintf(out_buff + pos, out_buff_size - pos, "| ");
	...
	static void log_message(int stack_trace_skip, Message& message, bool with_indentation, bool abort_if_fatal)
	{
		...
		if (message.verbosity == Verbosity_FATAL) {
			...
			if (!st.empty()) {
				RAW_LOG_F(0, ERROR, "Stack trace:\n%s", st.c_str());
			}

			auto ec = loguru::get_error_context();
			if (!ec.empty()) {
				RAW_LOG_F(0, ERROR, "%s", ec.c_str());
			}
		}
	...
	// stack_trace_skip is just if verbosity == FATAL.
	void log_to_everywhere(int stack_trace_skip, int sys, Verbosity verbosity,
	                       const char* file, unsigned line,
	                       const char* prefix, const char* buff)
	{
		char preamble_buff[LOGURU_PREAMBLE_WIDTH];
		print_preamble(preamble_buff, sizeof(preamble_buff), sys, verbosity, file, line);
		auto message = Message{sys, verbosity, file, line, preamble_buff, "", prefix, buff};
		log_message(stack_trace_skip + 1, message, true, true);
	}

#if LOGURU_USE_FMTLIB
	void log(int sys, Verbosity verbosity, const char* file, unsigned line, const char* format, fmt::ArgList args)
	{
		auto formatted = fmt::format(format, args);
		log_to_everywhere(1, sys, verbosity, file, line, "", formatted.c_str());
	}

	void raw_log(int sys, Verbosity verbosity, const char* file, unsigned line, const char* format, fmt::ArgList args)
	{
		auto formatted = fmt::format(format, args);
		auto message = Message{sys, verbosity, file, line, "", "", "", formatted.c_str()};
		log_message(1, message, false, true);
	}

#else
	void log(int sys, Verbosity verbosity, const char* file, unsigned line, const char* format, ...)
	{
		va_list vlist;
		va_start(vlist, format);
		auto buff = vtextprintf(format, vlist);
		log_to_everywhere(1, sys, verbosity, file, line, "", buff.c_str());
		va_end(vlist);
	}

	void raw_log(int sys, Verbosity verbosity, const char* file, unsigned line, const char* format, ...)
	{
		va_list vlist;
		va_start(vlist, format);
		auto buff = vtextprintf(format, vlist);
		auto message = Message{sys, verbosity, file, line, "", "", "", buff.c_str()};
		log_message(1, message, false, true);
		va_end(vlist);
	}
#endif
	...
	LogScopeRAII::LogScopeRAII(int sys, Verbosity verbosity, const char* file, unsigned line, const char* format, ...)
		: _sys(sys), _verbosity(verbosity), _file(file), _line(line)
	{
		if (verbosity <= current_verbosity_cutoff()) {
			...
			log_to_everywhere(1, sys, _verbosity, file, line, "{ ", _name);
			va_end(vlist);
	...
	LogScopeRAII::~LogScopeRAII()
	{
		if (_file) {
			...
			log_to_everywhere(1, _sys, _verbosity, _file, _line, "} ", buff.c_str());
#else
			log_to_everywhere(1, _sys, _verbosity, _file, _line, "}", "");
#endif
		}
	}
	...
	void log_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, const char* format, ...)
	{
		...
		va_start(vlist, format);
		auto buff = vtextprintf(format, vlist);
		log_to_everywhere(stack_trace_skip + 1, 0, Verbosity_FATAL, file, line, expr, buff.c_str());
		va_end(vlist);
...
namespace loguru
{
	...
	void signal_handler(int signal_number, siginfo_t*, void*)
	{
		...
		*/
		...
		flush();
		char preamble_buff[LOGURU_PREAMBLE_WIDTH];
		print_preamble(preamble_buff, sizeof(preamble_buff), 0, Verbosity_FATAL, "", 0);
		auto message = Message{0, Verbosity_FATAL, "", 0, preamble_buff, "", "Signal: ", signal_name};
		//try {
...
#undef MAX
	...
	struct Message
	{
		...
		int sys; // crossuo - submodule/system
	...
	*/
	...
	LOGURU_EXPORT
	void log(int sys, Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, fmt::ArgList args);
	FMT_VARIADIC(void, log, int sys, Verbosity, const char*, unsigned, LOGURU_FORMAT_STRING_TYPE)

	// Log without any preamble or indentation.
	LOGURU_EXPORT
	void raw_log(int sys, Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, fmt::ArgList args);
	FMT_VARIADIC(void, raw_log, int sys, Verbosity, const char*, unsigned, LOGURU_FORMAT_STRING_TYPE)
#else // LOGURU_USE_FMTLIB?
	// Actual logging function. Use the LOG macro instead of calling this directly.
	LOGURU_EXPORT
	void log(int sys, Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, ...) LOGURU_PRINTF_LIKE(5, 6);

	// Log without any preamble or indentation.
	LOGURU_EXPORT
	void raw_log(int sys, Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, ...) LOGURU_PRINTF_LIKE(5, 6);
	...
	public:
		...
		LogScopeRAII(int sys, Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, ...) LOGURU_PRINTF_LIKE(6, 7);
		~LogScopeRAII();
...
#define VLOG_F(sys, verbosity, ...)                                                                     \
	((verbosity) > loguru::current_verbosity_cutoff()) ? (void)0                                   \
									  : loguru::log(sys, verbosity, __FILE__, __LINE__, __VA_ARGS__)

// LOG_F(INFO, "Foo: %d", some_number);
#define LOG_F(sys, verbosity_name, ...) VLOG_F(sys, loguru::Verbosity_ ## verbosity_name, __VA_ARGS__)

#define VLOG_IF_F(sys, verbosity, cond, ...)                                                            \
	((verbosity) > loguru::current_verbosity_cutoff() || (cond) == false)                          \
		? (void)0                                                                                  \
		: loguru::log(sys, verbosity, __FILE__, __LINE__, __VA_ARGS__)

#define LOG_IF_F(sys, verbosity_name, cond, ...)                                                        \
	VLOG_IF_F(sys, loguru::Verbosity_ ## verbosity_name, cond, __VA_ARGS__)

#define VLOG_SCOPE_F(sys, verbosity, ...)                                                               \
	...
	loguru::LogScopeRAII(sys, verbosity, __FILE__, __LINE__, __VA_ARGS__)

// Raw logging - no preamble, no indentation. Slightly faster than full logging.
#define RAW_VLOG_F(sys, verbosity, ...)                                                                 \
	((verbosity) > loguru::current_verbosity_cutoff()) ? (void)0                                   \
									  : loguru::raw_log(sys, verbosity, __FILE__, __LINE__, __VA_ARGS__)

#define RAW_LOG_F(sys, verbosity_name, ...) RAW_VLOG_F(sys, loguru::Verbosity_ ## verbosity_name, __VA_ARGS__)

// Use to book-end a scope. Affects logging on all threads.
#define LOG_SCOPE_F(sys, verbosity_name, ...)                                                           \
	VLOG_SCOPE_F(sys, loguru::Verbosity_ ## verbosity_name, __VA_ARGS__)

#define LOG_SCOPE_FUNCTION(sys,verbosity_name) LOG_SCOPE_F(sys,verbosity_name, __func__)
...
#define CHECK_OP_F(expr_left, expr_right, op, ...)                                                 \
	...
	#define DLOG_F(sys, verbosity_name, ...)     LOG_F(sys, verbosity_name, __VA_ARGS__)
	#define DVLOG_F(sys, verbosity, ...)         VLOG_F(sys, verbosity, __VA_ARGS__)
	#define DLOG_IF_F(sys, verbosity_name, ...)  LOG_IF_F(sys, verbosity_name, __VA_ARGS__)
	#define DVLOG_IF_F(sys, verbosity, ...)      VLOG_IF_F(sys, verbosity, __VA_ARGS__)
	#define DRAW_LOG_F(sys, verbosity_name, ...) RAW_LOG_F(vsys, erbosity_name, __VA_ARGS__)
	#define DRAW_VLOG_F(sys, verbosity, ...)     RAW_VLOG_F(sys, verbosity, __VA_ARGS__)
#else
	// Debug logging disabled:
	#define DLOG_F(sys, verbosity_name, ...)
	#define DVLOG_F(sys, verbosity, ...)
	#define DLOG_IF_F(sys, verbosity_name, ...)
	#define DVLOG_IF_F(sys, verbosity, ...)
	#define DRAW_LOG_F(sys, verbosity_name, ...)
	#define DRAW_VLOG_F(sys, verbosity, ...)
#endif
